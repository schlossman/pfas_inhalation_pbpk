...
res.diff.calc <- function(out,acslx){
  ...
  print("Maximum percentage difference between template and acslx models (rounded to 0.01%).")
  print(paste("  in blood compartment:", max.diff(res_merge$bl, res_merge$blood_conc)))
  print(paste("    in fat compartment:", max.diff(res_merge$fat, res_merge$fat_conc)))
  print(paste(" in kidney compartment:", max.diff(res_merge$kid, res_merge$kidney_conc)))
  print(paste("  in liver compartment:", max.diff(res_merge$li, res_merge$liver_conc)))
}
